...
use ln::peers::handler::{SocketDescriptor, Queueable, Flushable};
...
impl Queueable for OutboundBuffer {
...
impl Flushable for OutboundBuffer {
...
mod tests {
	...
	fn empty_does_not_write() {
		...
		let mut empty = OutboundBuffer::new(10);
	...
	fn push_back_drain() {
		let mut descriptor = SocketDescriptorMock::new();
		let mut queue = OutboundBuffer::new(10);
	...
	fn push_back_above_limit_resume_reading_false() {
		...
		let mut queue = OutboundBuffer::new(1);
	...
	fn push_back_above_limit_is_ok() {
		let mut descriptor = SocketDescriptorMock::with_fixed_size(10);
		let mut queue = OutboundBuffer::new(2);
