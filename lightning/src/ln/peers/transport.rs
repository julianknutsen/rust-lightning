...
use ln::peers::handler::{ITransport, PeerHandleError, Queueable};
...
impl<PeerHandshakeImpl: IPeerHandshake> ITransport for Transport<PeerHandshakeImpl> {
	...
	fn enqueue_message<M: Encode + Writeable, Q: Queueable, L: Deref>(&mut self, message: &M, output_buffer: &mut Q, logger: L)
		where L::Target: Logger {
