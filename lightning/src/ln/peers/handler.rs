				...
				Some(peer) => {
					...
					match peer.transport.process_input(data, &mut peer.pending_outbound_buffer) {
						...
						Ok(_) => {
							...
							if peer.transport.is_connected() && !peers.node_id_to_descriptor.contains_key(&peer.transport.their_node_id.unwrap()) {
								peer.their_node_id = peer.transport.their_node_id;
