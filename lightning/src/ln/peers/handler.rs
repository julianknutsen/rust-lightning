...
pub(super) trait ITransport {
	...
	/// not established yet.
	fn enqueue_message<M: Encode + Writeable, Q: Queueable>(&mut self, message: &M, output_buffer: &mut Q);
}
				...
				Some(peer) => {
					...
					match peer.transport.process_input(data, &mut peer.pending_outbound_buffer) {
						...
						Ok(_) => {
							...
							if peer.transport.is_ready_for_encryption() && !peers.node_id_to_descriptor.contains_key(&peer.transport.their_node_id.unwrap()) {
